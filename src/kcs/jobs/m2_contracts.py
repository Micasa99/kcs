...
def _m2_document() -> dict[str, Any]:
    ...
    for candidate in candidates:
        ...
        try:
            ...
            raise RuntimeError("KCS 2.5 OpenAPI is unreadable") from error
    ...
    try:
        ...
        raise RuntimeError("KCS 2.5 OpenAPI is unavailable") from error
    if value.get("info", {}).get("version") != "2.5.0":
        raise RuntimeError("the served KCS package is not the frozen 2.5 contract")
    return value
...
def _validator(component: str) -> Draft202012Validator:
    ...
    if component not in document.get("components", {}).get("schemas", {}):
        raise RuntimeError(f"KCS 2.5 OpenAPI has no {component} component")
